...
.instruction-card { background:#F0F7F4; padding:24px; border-radius:16px; border-left:8px solid #2E7D20; margin-bottom:24px; }
</style>
""", unsafe_allow_html=True)

DEFAULTS = {
    "stage": "intro", "current_item_index": 0, "telemetry_logs": [],
    "item_start_time": None, "moca_naming_score": 0, "last_transcript": "",
}
...
ITEMS = [
    ...
    {"id":"item_2", "tier":"Moderate Familiarity", "emoji":"🐙", "primary_name":"八爪魚", "acceptable_synonyms":["八爪魚","章魚","呢個係八爪魚","這是八爪魚","呢隻係八爪魚"], "moca_weight":1},
...
def evaluate_answer(answer):
    ...
    st.session_state.telemetry_logs.append({
        ...
        "user_spoken_raw": answer, "is_correct": correct,
...
if st.session_state.stage == "intro":
    ...
    # Receive speech text before rendering the native Streamlit input.
    if "speech_result" in st.query_params:
        st.session_state.last_transcript = str(st.query_params["speech_result"])
    ...
    st.markdown("<p style='text-align:center;font-size:18px;color:#2E7D20;'>💡 提示：可以說<b>「呢個係...」</b></p>", unsafe_allow_html=True)
    st.markdown(f"<div style='font-size:130px;text-align:center;margin:10px 0;'>{item['emoji']}</div>", unsafe_allow_html=True)

    # HTML contains microphone controls only. Next and Skip are intentionally
    # outside this iframe as native Streamlit controls below.
    components.html("""
    <!doctype html><html><body style="margin:0;font-family:sans-serif">
    <style>
    #mic { width:100%;height:85px;font-size:24px;font-weight:bold;color:white;border:0;border-radius:18px;background:#E65100;cursor:pointer; }
    #status { font-size:20px;text-align:center;min-height:30px;margin-top:10px; }
    </style>
    <button id="mic" type="button">🎤 按此說話 (Tap & Say)</button>
    <div id="status">點擊上方按鈕並講出名稱</div>
    <script>
    ...
    function send(text) { const url=new URL(window.top.location.href); url.search=''; url.searchParams.set('speech_result',text); window.top.location.href=url.toString(); }
    if (SR) {
      ...
      recognition.onresult=(event)=>{ const text=event.results[0][0].transcript.trim(); if(!text){ready('⚠️ 沒有聽到內容 — 請再試一次');return;} navigating=true; status.textContent='✅ 聽到: '+text; mic.style.background='#388E3C'; send(text); };
    ...
    st.markdown("---")
    # Native Streamlit input: speech result is displayed here and can be edited.
    ...
    # Native Streamlit buttons only. They are deliberately not inside HTML.
    col1, col2 = st.columns(2)
    with col1:
        if st.button("👉 提交答案 / 下一題 (Submit / Next)", key=f"next_{index}"):
            correct = evaluate_answer(answer.strip() or "未有說話")
            if correct:
    ...
    with st.expander("🩺 Occupational Therapist / Speech Telemetry Dashboard", expanded=True):
        ...
        c1,c2=st.columns(2)
        with c1: st.metric("MoCA Proxy Naming Sub-score", f"{st.session_state.moca_naming_score} / {len(ITEMS)} Points")
        with c2:
