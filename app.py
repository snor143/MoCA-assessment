...
# Page configuration - Fixed layout to prevent native scrollbars
st.set_page_config(
    page_title="探索香港街市 (Explore HK Market)",
    page_icon="🛒",
    layout="wide",  # Wide layout to facilitate side-by-side components
    initial_sidebar_state="collapsed",
)

# Custom UI Styling with Dynamic Viewport Units (vh/vw/clamp)
st.markdown(
    """
<style>
/* Remove standard margins, paddings, and lock body height to prevent vertical scrolling */
...
def render_audio_speaker_component(words_list, key_suffix):
    """Cantonese Speech Synthesis component with responsive text."""
...
# --- STAGE 2: MEMORY REGISTRATION TRIAL 1 (PARALLEL LAYOUT) ---
    ...
    st.markdown(
        """
    <div class="instruction-card">
        <p style="font-size: clamp(16px, 2.4vh, 26px) !important; margin: 0;">請聽清楚並記住 5 個詞語，完畢後說出你記得的 (次序不限)：</p>
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    # Parallel 3-column layout to prevent vertical scrolling
...
# --- STAGE 3: MEMORY REGISTRATION TRIAL 2 (PARALLEL LAYOUT) ---
    ...
    st.markdown(
        """
    <div class="instruction-card">
        <p style="font-size: clamp(16px, 2.4vh, 26px) !important; margin: 0;">詞語會再播放一次，請再次講出記得的（包括剛才說過的）：</p>
    </div>
    """,
        unsafe_allow_html=True,
    )
...
# --- STAGE 6: DELAYED RECALL (FREE RECALL - PARALLEL) ---
    ...
    st.markdown(
        """
    <div class="instruction-card">
        <p style="font-size: clamp(18px, 2.8vh, 30px) !important; margin: 0;"><b>請講出最開始播放過給您聽的那 5 個詞語：</b></p>
    </div>
    """,
        unsafe_allow_html=True,
    )
