...
# --- STAGE 2: GAMEPLAY ---
    ...
    current_key = f"input_widget_{index}"

    # Catch incoming transcript from query parameters
    ...
    # Dedicated Speech Recognition HTML Component
    components.html(
        f"""
    <!doctype html>
    <!-- Item Index: {index} -->
    ...
    with col1:
        if st.button("👉 提交答案 (Submit)", key=f"btn_sub_{index}"):
            ans = user_input.strip() if user_input.strip() else "未有說話"
    ...
    with col2:
        if st.button("⏭️ 跳過 (Skip)", key=f"btn_skip_{index}"):
            evaluate_answer("跳過")
            advance_item()
            st.rerun()
